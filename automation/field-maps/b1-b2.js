// ============================================================
// B1/B2 Field Map — Tourist/Business visa DS-160
// ============================================================
const { ph, isPostbackSelect, isPostbackClick, POSTBACK_SELECT_IDS, POSTBACK_CLICK_YES_IDS, POSTBACK_CLICK_ANY_IDS } = require('./shared');
...
function buildDynamicFieldMap(a) {
  ...
  map.push(
    ...
    { pattern: /ddlOtherPurpose$/i, value: a.purposeSubCategory || a.purposeOfTrip, type: "select" },
  ...
  if (a.travelingWithOthers && a.companions?.length) {
    ...
    a.companions.forEach((comp, idx) => {
      ...
      // Fallback específico para ctl00 (IDs podem variar, mas sempre conter TC_SURNAME)
      if (idx === 0) {
        map.push(
  ...
  if (a.hasUSVisa && a.previousVisa) {
    ...
    if (visa.lost) {
      if (visa.lostYear) map.push({ pattern: /tbxLOST_VISA_YEAR$/i, value: visa.lostYear, type: "text" });
      if (visa.lostExplanation) map.push({ pattern: /tbxLOST_VISA_EXPL$/i, value: visa.lostExplanation, type: "text" });
    }
    map.push(
    ...
    if (visa.cancelled && visa.cancelledExplanation) {
      map.push(
        ...
        { pattern: /tbxCANCELLED_VISA_EXPL$/i, value: visa.cancelledExplanation, type: "text" }, // fallback
      );
    }
  } else {
  ...
  const langs = a.languages?.length > 0 ? a.languages : ['PORTUGUESE'];
