...
// Generated: 2026-03-18T06:04:30.326Z
...
async function waitForPostback(page, timeout = 3000) {
    ...
    await page.waitForFunction(() => {
        var mgr = window.Sys?.WebForms?.PageRequestManager?.getInstance?.();
...
async function waitForPageReady(page, timeout = 2000) {
    ...
    while (Date.now() - start < timeout) {
        ...
        var { count, inPB } = await page.evaluate(() => {
            ...
            var m = window.Sys?.WebForms?.PageRequestManager?.getInstance?.();
...
async function fillText(page, fieldId, value) {
    var loc = page.locator(`#${fieldId.replace(/\$/g, '\\$')}`);
    var isVis = await loc.isVisible({ timeout: 300 }).catch(() => false);
...
async function fillSelect(page, fieldId, value, type = 'select') {
    ...
    if (type === 'select-search' || type === 'select-label') {
        ...
        var allOpts = await loc.evaluate(sel =>
            Array.from(sel.options).map(o => ({ v: o.value, t: o.text }))
        );
        var found = allOpts.find(o => o.t.toUpperCase() === String(value).toUpperCase());
    ...
    var MONTH_ABBREV = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
    var MONTH_SET = new Set(MONTH_ABBREV);
    ...
    try { await loc.selectOption(strVal); return true; }
        ...
        try { await loc.selectOption({ label: strVal }); return true; }
            ...
            try {
                var opts = await loc.evaluate(sel => Array.from(sel.options).slice(0, 8).map(o => `${o.value}="${o.text}"`).join(', '));
...
async function fillRadio(page, fieldId, value) {
    var suffix = value === 'Y' ? '_0' : '_1';
    var baseId = fieldId.replace(/_(0|1)$/, '');
    var targetId = baseId + suffix;
    var loc = page.locator(`#${targetId.replace(/\$/g, '\\$')}`);
    var isVis = await loc.isVisible({ timeout: 300 }).catch(() => false);
    if (!isVis) return false;
    var alreadyChecked = await loc.isChecked().catch(() => false);
    if (!alreadyChecked) {
        await loc.click();
    }
    return true;
...
async function dismissTooltip(page) {
    await page.evaluate(() => {
        ...
        var tooltip = document.getElementById('ctl00_ToolTipManager1');
        if (tooltip) tooltip.style.display = 'none';
        var bubble = document.getElementById('bubble_tooltip_content');
        if (bubble) {
            var parent = bubble.closest('[style]');
...
async function clickAddAnother(page, listName, targetIdx) {
    var targetCtl = `_ctl${String(targetIdx).padStart(2, '0')}_`;
    var targetSelector = `[id*="${listName}"][id*="${targetCtl}"]`;
    var clicked = false;
    ...
    try {
        var insertBtns = await page.locator(`[id*="${listName}"][id*="InsertButton"]`).all();
        for (let i = insertBtns.length - 1; i >= 0; i--) {
            var btn = insertBtns[i];
            if (await btn.isVisible({ timeout: 500 }).catch(() => false)) {
                ...
                var btnId = await btn.getAttribute('id').catch(() => '');
    ...
    if (!clicked) {
        try {
            var addLinks = await page.locator(`a:has-text("Add Another")`).all();
            for (const link of addLinks) {
                if (!await link.isVisible({ timeout: 500 }).catch(() => false)) continue;
                var nearList = await link.evaluate((el, ln) => {
    ...
    if (!clicked) {
        try {
            var clickedViaJS = await page.evaluate((ln) => {
                // 3a: InsertButton com listName no ID
                var insertBtns = document.querySelectorAll(`a[id*="${ln}"][id*="InsertButton"]`);
                ...
                for (const a of allLinks) {
                    ...
                    var isAddAnother = txt === 'Add Another' || a.classList.contains('addone');
                    var refsDataList = href.includes(ln) || onclick.includes(ln);
                ...
                var dataListEl = document.querySelector(`[id*="${ln}"]`);
                if (dataListEl) {
                    var parent = dataListEl.parentElement;
                    for (let i = 0; i < 10 && parent; i++) {
                        var addLink = parent.querySelector('a.addone, a[id*="InsertButton"]');
                        if (addLink) {
                            addLink.click();
                            return '3d-proximity';
                        }
    ...
    try {
        ...
        await page.evaluate((ln) => {
            var links = document.querySelectorAll(`a[id*="${ln}"][id*="InsertButton"]`);
    ...
    var empty = await page.evaluate(() => {
        ...
        document.querySelectorAll('select').forEach(sel => {
            ...
            if (!sel.value || sel.value === '' || sel.value === '-1') {
                var shortId = sel.id.split('_').pop();
        ...
        document.querySelectorAll("input[type='text']").forEach(inp => {
            ...
            if (!inp.value || inp.value.trim() === '') {
                var shortId = inp.id.split('_').pop();
                emptyIds.push(shortId);
            }
        });
        // Textareas vazias
        ...
        for (const name of radioGroups) {
            ...
            if (!checked) {
                ...
                var shortId = first?.id?.split('_').pop() || name;
...
var SKIP_PATTERN = /HelpButton|btnWarning|btnRecover|btnOkWarning|btnCancel|btnClient|btnReviewPage|btnNextPage|btnModalHolder/;
...
function buildFieldIndex(fieldMap, visibleFields) {
    var index = new Map(); // fieldId → fieldMap entry
...
async function fillPage(page, fieldMap, options = {}) {
    var { maxPasses = 10 } = options;
    await waitForPageReady(page);
    var pageStart = Date.now();
    var pass = 0, needsRescan = true;
    var postbackLog = [];
    var addAnotherClicked = new Set();
    var postbackSelectsFilled = new Set();

    while (needsRescan && pass < maxPasses) {
        var result = await _fillPass(page, fieldMap, pass, addAnotherClicked, postbackSelectsFilled);
    ...
    var removedCount = await page.evaluate(() => {
        var removed = 0;
        ...
        for (const inp of allInputs) {
            ...
            for (let i = 0; i < 15 && container; i++) {
                var removeLink = container.querySelector('a[id*="DeleteButton"], a[id*="RemoveButton"]');
                if (removeLink) {
                    removeLink.click();
                    removed++;
                    break;
    ...
    var elapsed = ((Date.now() - pageStart) / 1000).toFixed(1);
    if (postbackLog.length > 0) {
        console.log(`[Page] Postbacks: ${postbackLog.join(' → ')}`);
    }

    // Detect empty fields
    var emptyCheck = await verifyPageFields(page);
    if (!emptyCheck.ok) {
...
async function verifyPage(page) {
    var check = await verifyPageFields(page);
...
async function _fillPass(page, fieldMap, passNum, addAnotherClicked, postbackSelectsFilled) {
    ...
    var fields = await discoverFields(page);
    var visible = fields.filter(f => f.visible && f.id);
    var postbackNeeded = false, filled = 0;
    var postbackField = null;
    var fieldsBeforeCount = visible.length;

    // Pre-compute match index (feito UMA VEZ por pass, evita O(n×m) nas 4 fases)
    var matchIndex = buildFieldIndex(fieldMap, visible);

    // ======================== PHASE 1: Postback Clicks ========================
    // DEBUG: count candidates at each filter step
    var p1_total = 0, p1_matched = 0, p1_click = 0, p1_postback = 0, p1_unchecked = 0;
    for (const field of visible) {
        if (!field.id) continue;
        if (field.type === 'submit' || field.type === 'image' || field.type === 'button') continue;
        p1_total++;
        var match = matchIndex.get(field.id);
        ...
        var loc = page.locator(`#${field.id.replace(/\$/g, '\\$')}`);
        try {
    ...
    if (!postbackNeeded) {
        for (const field of visible) {
            ...
            try {
                ...
                var ok = await fillSelect(page, field.id, match.value, match.type);
                if (ok) {
                    filled++;
                    postbackNeeded = true;
                    postbackField = field.id;
    ...
    var addAnotherEntries = fieldMap.filter(m => m.addAnother);
    if (addAnotherEntries.length > 0) {
        var pendingByList = {};
        // Group entries by list:idx — only need AddAnother if NO field for this ctl exists
        var checkedCtls = new Set();
        for (const entry of addAnotherEntries) {
            var listName = entry.addAnother.list;
            var idx = entry.addAnother.idx;
            var trackKey = `${listName}:${idx}`;
            ...
            var ctlStr = `_ctl${String(idx).padStart(2, '0')}_`;
            var ctlExists = visible.some(f => f.id && f.id.includes(listName) && f.id.includes(ctlStr));
            if (!ctlExists) {
        ...
        for (const [listName, entry] of Object.entries(pendingByList)) {
            var trackKey = `${listName}:${entry.addAnother.idx}`;
            var listClickCount = [...addAnotherClicked].filter(k => k.startsWith(listName + ':')).length;
            if (listClickCount >= 5) {
            ...
            var prevIdx = entry.addAnother.idx - 1;
            var prevCtl = `_ctl${String(prevIdx).padStart(2, '0')}_`;
            var prevExists = visible.some(f => f.id && f.id.includes(listName) && f.id.includes(prevCtl));
            if (!prevExists && prevIdx > 0) continue;

            console.log(`[Page] 📋 Add Another: "${listName}" (idx ${entry.addAnother.idx})`);
            var ok = await clickAddAnother(page, listName, entry.addAnother.idx);
            addAnotherClicked.add(trackKey);
    ...
    for (const field of visible) {
        ...
        try {
            switch (match.type) {
                ...
                case 'click':
                    if (field.checked) continue;
                    var clickLoc = page.locator(`#${field.id.replace(/\$/g, '\\$')}`);
    ...
    if (textBatch.length > 0) {
        ...
        var CRITICAL = /Address|Street|City|Phone|Payer|Employer|Salary|Income|Occupation/i;
        var criticalBatch = textBatch.filter(f => CRITICAL.test(f.id));
        var normalBatch = textBatch.filter(f => !CRITICAL.test(f.id));
        ...
        if (normalBatch.length > 0) {
            ...
            var batchFilled = await fillTextBatch(page, normalBatch);
            filled += batchFilled;
        }
    }
...
async function fillTravelPage(page, fieldMap, profile) {
    ...
    var t = profile.travel || {};
    var hasSpecific = profile.hasSpecificPlans;
    ...
    var otherPurposeVal = profile.purposeSubCategory || (profile.purposeOfTrip || '').replace(/\//g, '-');
    if (otherPurposeVal) {
        await setSelectIfNeeded(page, 'ddlOtherPurpose', otherPurposeVal, true);
    }

    // 1c. SpecificTravel (POSTBACK) — Yes/No
    var specificRadioSuffix = hasSpecific ? 'rblSpecificTravel_0' : 'rblSpecificTravel_1';
    var specificRadio = page.locator(`[id$='${specificRadioSuffix}']`).first();
    ...
    if (hasSpecific && t.arrivalDate) {
        ...
        var locs = (profile.specificLocations || []).map(l => typeof l === 'object' ? (l.location || l.name || '') : String(l));
        if (locs.length > 0) {
    ...
    for (const { suffix, value } of selectBatch) {
        if (!value) continue;
        var sel = page.locator(`[id$='${suffix}']`).first();
        try {
    ...
    if (payer && ['O', 'C', 'P', 'U'].includes(profile.payingForTrip)) {
        ...
        if (profile.payingForTrip === 'O') {
            ...
            if (payer.email) {
                ...
                var dnaEmail = page.locator("[id$='cbxDNAPAYER_EMAIL_ADDR_NA']").first();
        ...
        if (profile.payingForTrip === 'O' && payer.relationship) {
            var relSel = page.locator("[id$='ddlPayerRelationship']").first();
        ...
        if (profile.payingForTrip === 'O') {
            ...
            var sameAddrRaw = payer.sameAddress;
            var sameAddr = sameAddrRaw === true || sameAddrRaw === 'Y';
            var sameRadioId = sameAddr ? 'rblPayerAddrSameAsInd_0' : 'rblPayerAddrSameAsInd_1';
            var sameRadio = page.locator(`[id$='${sameRadioId}']`).first();
        ...
        var needsAddress = ['C', 'P', 'U'].includes(profile.payingForTrip) || (profile.payingForTrip === 'O' && !(payer.sameAddress === true || payer.sameAddress === 'Y'));
        if (needsAddress) {
            var addrBatch = [];
            ...
            if (payer.country) {
                var countrySel = page.locator("[id$='ddlPayerCountry']").first();
                if (await countrySel.isVisible({ timeout: 500 }).catch(() => false)) {
                    var allOpts = await countrySel.evaluate(sel =>
                        Array.from(sel.options).map(o => ({ v: o.value, t: o.text }))
                    );
                    var found = allOpts.find(o => o.t.toUpperCase() === payer.country.toUpperCase());
    ...
    // ================================================================
    if (hasSpecific) {
        var locs2 = (profile.specificLocations || []).map(l => typeof l === 'object' ? (l.location || l.name || '') : String(l));
        for (let i = 1; i < locs2.length; i++) {
            var newCtl = `ctl${String(i).padStart(2, '0')}`;
            var locSelector = `[id$='dtlTravelLoc_${newCtl}_tbxSPECTRAVEL_LOCATION']`;
            
            // Check if field already exists (from previous attempt/retry)
            var alreadyExists = await page.locator(locSelector).first().isVisible({ timeout: 300 }).catch(() => false);
            
            if (!alreadyExists) {
                // Only click Add Another if the field doesn't exist yet
                var addBtn = page.locator("[id$='InsertButtonTravelLoc']").first();
            ...
            var loc = page.locator(locSelector).first();
            if (await loc.isVisible({ timeout: 300 }).catch(() => false)) {
                var curVal = await loc.inputValue().catch(() => '');
...
async function setSelectIfNeeded(page, idSuffix, value, isPostback = false) {
    var sel = page.locator(`[id$='${idSuffix}']`).first();
    if (!await sel.isVisible({ timeout: 500 }).catch(() => false)) return false;
    var curVal = await sel.evaluate(el => el.value).catch(() => '');
